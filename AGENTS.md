...
```bash
mvn package                                  # produces target/SimpliCity-0.1.jar
java -jar target/SimpliCity-0.1.jar          # run the game
...
- `micropolisj.build_tool.MakeTiles` — regenerates the composite tile sheet from individual tile PNGs and `src/main/resources/tiles.rc`. Honors `-Dtile_size=`, `-Dskip_tiles=`, `-Dtile_count=` system properties.
...
- `src/main/resources/tiles.rc` — declarative tile definitions consumed by `TileSpec` at runtime and by `MakeTiles` at build time.
- `src/main/resources/{8x8,16x16,32x32}/` — pre-rendered tile sheets at three zoom levels.
- `src/main/resources/sounds/` — game audio.
- `tiles/` and `graphics/` (repo root, outside `src/`) — source artwork used by `MakeTiles` to regenerate the resource sheets.
