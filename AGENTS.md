...
- `micropolisj.build_tool.GenerateSpriteArt` — draws the mobile sprites (`obj*_{32x32,64x64}.png`: train, helicopter, airplane, ship, monster, tornado, explosion, bus) as real hi-res art. The classic skin keeps pixel-art upscales of the originals under `resources/classic/` (preferred by `TileImages`); the 16px base `obj*.png` stays the original art everywhere.
- `micropolisj.build_tool.GenerateDeluxeArt` — produces the deluxe skin's building sheets (`graphics/deluxe/gen/`). Every building sheet is drawn from scratch at 64px/tile: `DeluxeResArt` (residential), `DeluxeComArt` (commercial), `DeluxeIndArt` (industrial zones plus piston/smoke frames), `DeluxeCivicArt` (fire/police stations plus their beacon-light animation frames — new always-on animations wired via `graphics/tiles.rc` tiles 763/771 and the `*_light*_animation.ani` files; their 16px base sheets are generated into `graphics/` so classic/modern compose them too), `DeluxeStadiumArt` (stadium, full stadium, football-game frames), `DeluxePowerArt` (coal plant + smoke frames, nuclear plant + reactor swirl), `DeluxePortArt` (seaport, airport, radar frames). Only `misc_animation` still goes through the legacy upscaler (checker pre-blend + edge-aware mean-shift 4x + saturation/grain), as the base `DeluxeMiscArt` patches its redrawn tiles into (power-out indicator, drawbridges, destruction explosion, radar, swirl).
- `micropolisj.build_tool.GenerateToolIcons` — draws the toolbar tool icons (`ic*.png` / `ic*hi.png`) as stylized glyphs for the modern and deluxe skins, at the exact per-tool pixel sizes the classic icons use. Outputs `resources/tools/` (modern) and `resources/deluxe/tools/` (deluxe); the classic skin keeps the original root `ic*.png`. The GUI (`MainWindow.toolIconResource`) resolves icons per skin and `refreshToolIcons()` reloads them when the graphics theme changes.
...
- `src/main/resources/{8x8,16x16,32x32,64x64}/` — pre-rendered tile sheets at four zoom levels (zoom range 8–64; `MicropolisDrawingArea` is HiDPI-aware and picks the sheet covering `zoom × device scale`). `sm/` holds the 3px minimap tiles. `classic/{N}x{N}/` holds the original 80s-style art and `deluxe/{N}x{N}/` the redrawn deluxe skin (falls back to modern art for sheets not yet redrawn in `graphics/deluxe/`), both selectable live via Options → Graphics (`TileSkin` preference).
...
- Existing files that already have the original GPLv3 / EA copyright header — keep it when editing them. Do **not** add copyright headers to new files.
